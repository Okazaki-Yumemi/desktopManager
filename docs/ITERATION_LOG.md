## 2026-09-12 — Round 29: 交大日程多周同步（用户报告：周日看不到下周一）

- **问题**：门户日历 SPA 一次只给一周数据，注入脚本只转发第一个匹配
  响应，后端整表替换 + 1.5s 后自动关窗 —— 结果永远只有当前一周，
  周日无法提前看到下周一的课。
- **修复（三层）**：①注入脚本去掉一次性开关，凡匹配日程形状的响应
  都推送（按内容去重）；捕获首个响应的请求 URL 作模板，把其中的日期
  参数（路径/查询里的 ISO 日期、查询里的 epoch 秒/毫秒）逐周 +7 天
  偏移，自动向后拉 8 周（不可偏移的 URL 退化为手动翻周，翻页响应同
  样被捕获）。②后端 `sjtu_receive` 从 replace_all 改为
  `upsert_events`（按 external_id ON CONFLICT 合并；已结束超 7 天的行
  同事务修剪——上一周保留可回看，整学期推送量有界）。自动关窗改为
  静默 6 秒去抖（LAST_RECEIVE 原子戳，每推一次重置），给 8 次自动
  拉取和手动翻周留时间。③报告增加 `total`（会话累计），前端 toast
  10s 去抖、显示累计而非单周条数。
- **门禁**：cargo test 69/69（新增合并+更新+修剪、保留窗口两组测试
  ——第一版 2 天保留期会被刚推送的上周数据踩掉，测试当场暴露，改
  为 7 天）、clippy 0、svelte-check 0/0、eslint 0、tauri build exit 0
  （MSI 3.8 MB / NSIS 2.8 MB / zip 3.4 MB / exe 7.4 MB）。v1.3.1。
- **验证边界（如实）**：仓库层合并/修剪为单元测试覆盖；注入脚本的
  真实拉取链路需要 jAccount 会话，须由用户重新同步一次验证（打开
  同步窗口登录即可，无需任何额外操作；同步完成后窗口在最后一次推
  送 6 秒后自动关闭）。脚本变更标记 TESTED（结构）而非
  WINDOWS_TESTED，待用户验证后升级。
...
- **命令面板**（Ctrl+K / 侧栏「快速操作」入口）：过滤框（标签中文 +
  关键词英文双路匹配、空态提示）、↑↓/回车/Esc 全键盘操作、悬停同步
  选中；动作含六个页面直达、番茄 25 / 深度 50 一键专注（写库后跳转
  专注页）、记任务、主题三态切换、交大日程同步、导出 ICS、换壁纸
  （库 ≥2 张时出现）。
- **键盘**：日历页 ←/→ 翻周/月、T 回今天（输入框聚焦时让路）；全局
  Ctrl+1..6 直达第 N 页（输入框让路）。
- **版本**：三处升级 1.2.0，页脚 v1.2.0 · M14。
- 门禁：svelte-check 0/0、eslint 0、vite build ok（195.87 kB）、
  cargo test 64/64、clippy 0、tauri build exit 0（MSI 3.2 MB /
  NSIS 2.3 MB / zip 5.2 MB / exe 6.3 MB）。
- **真机验证（WINDOWS_TESTED）**：Ctrl+K 面板打开/过滤/执行直达设置；
  今天页概览卡（2/8 任务、2 日程、0 专注）实时数据；日历周视图事件
  块带开始时间、全天 MCM、自动滚到当前时段——用户夜间已实际用拖拽
  创建了一批 MCM 时段日程（操作性被真实使用验证）；壁纸库预置两张
  种子图经 `bg://library/` 协议出缩略图，点击应用后全窗背景即时切换
  为蓝色种子图、当前项高亮环、自动切换六档+顺序/随机渲染正确。
  注意：应用过程覆盖了 background.img（单文件设计），用户原壁纸需
  重新添加一次——已如实告知用户。

## 2026-09-07 — Round 26: 日历页 UI 重做（用户点名，易读性/操作性优先）

- **布局重构**：主视图 + 右侧常驻「当日面板」（300px，窄屏自动堆叠），
  交大日程侧栏移入右栏下方；底部议程条取消——当天的条目、编辑表单、
  新建入口全部收进面板，所见即所指。
- **操作性**：周视图时间格支持**按下拖动跨选小时**（拖拽区间实时显示
  时间范围）→ 松开即带时长打开创建表单；悬停浅高亮提示可点；事件块
  点击即编辑（原版只能删）；全天行悬停出现 + 新建全天日程。
- **编辑能力打通**：`event_update` 首次接入前端——标题/开始/时长/全天/
  关联任务/备注全部可改、可删除（SJTU 条目保持只读标签）。
- **易读性**：事件块高度 ≥34px 时显示开始时间；选中事件块与面板条目
  双向高亮；HOUR_H 40→44；打开页面/点「今天」自动滚到当前时段上两格。
- 门禁：svelte-check 0/0、eslint 0、vite build ok（190.34 kB）。真机
  验证随 R27 构建后统一进行。
- 设计取舍（D27）：不改后端与数据模型；网格 div 采用 pointer 事件而非
  每格 button（键盘路径由事件块的 tabindex + Enter 与面板承接）。
...
- **自定义座右铭**：`ui.mottos`（string[]，≤50 条）；设置页新增
  「今天页」分区（输入回车/按钮添加、芯片删除）；非空则每天从
  自定义池轮换（按天序取模），空则回落内置十句。存取失败仅影响
  持久化，会话内即时生效。
- **时钟风格**：`ui.clockStyle` ∈ 经典（原渐变+秒）/ 极简（细体无秒）/
  翻页（数字卡 rotateX 翻转入场，keyed-by-value 触发，冒号呼吸）/
  卡片（每位数字独立瓷片+小秒）；设置页分段控件即时切换。
- **首页概览条**：三张玻璃卡（任务完成 x/y、今日日程 n、专注分钟），
  数据来自现有 listTasks / listEventsRange / getFocusSummary(1)，
  失败显示 —，点击跳转对应页。
- 门禁：svelte-check 0/0、eslint 0、vite build ok（179.75 kB）。

## 2026-09-06 — Round 23: v1.1.0 前端全面美化（用户点单）

- **全局基座**：tokens 新增阴影三档（--shadow-sm/md/lg）、--grad-accent
  （基于 --accent 的 color-mix 渐变，自定义强调色自动跟随）、
  --accent-contrast（深色主题下主按钮改用深色文字，修掉白字配浅蓝底的
  对比度问题）、--accent-ring 焦点光环；base.css 补上**此前根本不存在
  的全局 .glass 工具类**（交大侧栏与当日议程面板一直是透明底）、
  :focus-visible 全局焦点态、选区颜色、悬浮滚动条、page-in/toast-in
  动画库（data-motion=off 与 prefers-reduced-motion 自动禁用）。
- **侧栏/提醒**：导航逐项配 lucide 图标 + 渐变品牌标（SVG 四格）；
  Toast 加类型图标 + 左侧色条 + 滑入动画 + 悬停位移。
- **今天页**：时钟渐变文字（含 @supports 回退）、双氛围光斑、状态
  药丸加圆点。
- **专注页**：倒计时改为 SVG 圆环进度（focus=accent / break=ok /
  超=warn），主按钮渐变化 + 悬浮抬升，近 7 天条形图渐变填充。
- **日历页（结构性重构，D25）**：周视图改为「时刻度列 + 7 天列 ×
  表头/全天/时段三行」共享网格——修掉某天有全天日程时各列时段网格
  错位的旧问题，并补上此前完全缺失的小时刻度；今天列高亮、周末浅
  染、红色现时线加端点圆；今天日期渐变圆徽；月视图芯片与议程行
  打磨；创建面板改浮起卡片。
- **桌面/任务/设置**：卡片悬停抬升 + 选中内环、图标底座、空态虚线
  面板、截止日期药丸化；设置页九个分区加图标、分段控件 Fluent 化
  （白底浮起活动块）；清理「任务与日历将随 M6 到来」等陈旧文案。
- **质量门**：svelte-check 0/0（修掉 3 条 scoped 选择器警告——lucide
  图标需 :global(svg)）、eslint 0、vite build ok、cargo test 63/63、
  clippy 0。`tauri build` exit 0：MSI 3.2 MB / NSIS 2.3 MB /
  zip 5.2 MB / exe 6.3 MB。
- **真机验证（WINDOWS_TESTED）**：用户在场并亲自把玩设置页；其余
  页面经后台 a11y 点击 + PrintWindow 遮挡抓图逐页核对（今天页居中
  用 UIA 边界复核实测精确居中）。日历周视图时刻度对齐、现时线、
  交大侧栏「正在上课」倒计时（还剩 10→9 分钟）均确认。
- 版本号三处升级 1.1.0，侧栏页脚 v1.1.0 · M13。
