...
## D29 — 多周同步：合并而非替换，静默去抖关窗（2026-09-12, R29）

**问题**：门户日历 API 一次返回一周（被动 hook 拿到的就是 SPA 首屏那
一周），旧实现"第一个响应 → 整表替换 → 1.5s 关窗"把多周的可能性
从三个环节都堵死了。用户周日安排下周一的需求直接撞墙。

**决定**：
1. **合并语义**。`sjtu_receive` 从"整表替换"改为按 external_id
   UPSERT（迁移 0009 的 UNIQUE 约束正好支撑）。代价是失去"重同步必
   无陈旧行"的强保证：服务端取消/改期的未来事件会滞留到自然过期
   （按旧 external_id 保留旧副本）。这是换取多周合并的诚实代价，记
   录在案；作为补偿，每次合并同事务删除已结束超 7 天的行，投影规
   模有界、上周仍可回看。
2. **自动拉未来 8 周**。脚本捕获首个响应的请求 URL 作模板，对其中
   可识别的日期参数做 +7天×k 偏移重放（同源、带凭据、串行、去重）。
   不去猜门户未公开的 API 约定——偏移不了就退化成"用户手动翻周，
   脚本照单全收"，两条路都通。
3. **关窗去抖**。关窗时点从"首推后 1.5s"改为"最后一次推送后 6s"（原
   子时间戳比较实现，不引入新线程泄漏）。手动关窗路径不变。
4. **进度诚实**。报告带会话累计 `total`，前端 toast 去抖 10s 且只报
   累计——不出现"35 条"×9 的假象，也不谎报一次完成。
